<script>
   ...
   *
   * Admin-only Settings section for personal access token management.
   * Currently the sole consumer is the MCP endpoint (/api/mcp). Lists
   * existing tokens, lets the admin create new ones (with name + scope
   * checkboxes + optional expiry), and revoke them.
   *
   ...
   *
   * See docs/cooktrace/mcp.md for what these tokens unlock.
   */
  ...
  let newName = '';
  let newScopes = new Set(['mcp:read']);
  ...
  async function load() {
    ...
    try {
      ...
      if (!r.ok) throw new Error($_('settings_api_tokens_ct.toast.load_failed'));
  ...
  async function createNewToken() {
    if (creating) return;
    if (!newName.trim()) { showError($_('settings_api_tokens_ct.toast.name_required')); return; }
    if (newScopes.size === 0) { showError($_('settings_api_tokens_ct.toast.scope_required')); return; }
    ...
    try {
      ...
      const data = await r.json();
      if (!r.ok) { showError(data.error || $_('settings_api_tokens_ct.toast.create_failed')); return; }

      justCreatedRaw = data.raw;
      justCreatedName = data.token.name;
      showCreateForm = false;
      newName = '';
      newScopes = new Set(['mcp:read']);
      newExpiresDays = '';
      await load();
    } catch (e) {
      showError(e.message);
  ...
  async function revokeOne(t) {
    if (!await confirmDialog({
      title: $_('settings_api_tokens_ct.confirm.revoke_title', { values: { name: t.name } }),
      message: $_('settings_api_tokens_ct.confirm.revoke_msg'),
      confirmText: $_('settings_api_tokens_ct.confirm.revoke_confirm'),
      dangerous: true,
    })) return;
    try {
      ...
      if (!r.ok) {
        ...
        showError(data.error || $_('settings_api_tokens_ct.toast.revoke_failed')); return;
      }
      showSuccess($_('settings_api_tokens_ct.toast.revoked'));
      await load();
    } catch (e) {
      showError(e.message);
    }
  ...
  async function copyRaw() {
    try {
      ...
      showSuccess($_('settings_api_tokens_ct.toast.copied'));
    } catch {
      showError($_('settings_api_tokens_ct.toast.copy_failed'));
    }
  }
  ...
  function _fmtRelative(iso) {
    if (!iso) return $_('settings_api_tokens_ct.row.never');
    const d = new Date(iso.includes('T') ? iso : iso.replace(' ', 'T') + 'Z');
    if (isNaN(d)) return iso;
    const ms = Date.now() - d.getTime();
    const sec = Math.floor(ms / 1000);
    if (sec < 60) return $_('settings_api_tokens_ct.row.just_now');
    if (sec < 3600) return $_('settings_api_tokens_ct.row.min_ago', { values: { n: Math.floor(sec / 60) } });
    if (sec < 86400) return $_('settings_api_tokens_ct.row.hr_ago',  { values: { n: Math.floor(sec / 3600) } });
    if (sec < 86400 * 30) return $_('settings_api_tokens_ct.row.day_ago', { values: { n: Math.floor(sec / 86400) } });
...
<div class="section-body">
  <p class="sub-label" style="padding:0 0 6px">
    {$_('settings_api_tokens_ct.intro_before')} <em>{$_('settings_api_tokens_ct.intro_dev')}</em>){$_('settings_api_tokens_ct.intro_middle')}
    <code>/api/mcp</code>. {$_('settings_api_tokens_ct.intro_see')}
    <a href="https://traceapps.github.io/docs/cooktrace/mcp/" target="_blank" rel="noopener">docs/cooktrace/mcp.md</a>.
  </p>
  ...
  {#if justCreatedRaw}
    <div class="just-created" transition:slide={{ duration: 160 }}>
      <div class="just-created-title">
        ...
        {$_('settings_api_tokens_ct.just_created.title')} <strong>{justCreatedName}</strong>
      </div>
      <p class="just-created-warn">
        {$_('settings_api_tokens_ct.just_created.warn')}
      </p>
      <div class="just-created-row">
        ...
        <button class="btn btn-secondary" style="height:32px;font-size:12px;padding:0 12px" on:click={copyRaw}>
          ...
          {$_('settings_api_tokens_ct.just_created.copy')}
        </button>
      </div>
      <button class="btn btn-ghost" style="margin-top:8px;width:100%" on:click={dismissJustCreated}>
        {$_('settings_api_tokens_ct.just_created.saved')}
      </button>
    </div>
  {/if}
  ...
  <div class="card settings-card">
    ...
    {:else if tokens.length === 0}
      <div class="setting-row" style="flex-direction:column;align-items:flex-start;gap:4px;padding:14px 16px">
        <span class="setting-label">{$_('settings_api_tokens_ct.empty.title')}</span>
        <span class="setting-desc">{$_('settings_api_tokens_ct.empty.desc')}</span>
      </div>
    {:else}
        ...
        <div class="token-row">
          <div class="token-info">
            ...
            <span class="token-meta text-3 text-sm">
              {$_('settings_api_tokens_ct.row.scopes_label')} {t.scopes.join(', ') || $_('settings_api_tokens_ct.row.scopes_none')}
              · {$_('settings_api_tokens_ct.row.last_used', { values: { when: _fmtRelative(t.last_used_at) } })}
              {#if t.expires_at} · {$_('settings_api_tokens_ct.row.expires', { values: { when: _fmtRelative(t.expires_at) } })}{/if}
            </span>
          </div>
          <button class="btn-icon" title={$_('settings_api_tokens_ct.row.revoke_title')} on:click={() => revokeOne(t)}>
    ...
    <div style="padding:12px 16px">
      {#if !showCreateForm}
        <button class="btn btn-secondary" style="width:100%" on:click={() => showCreateForm = true}>
          ...
          {$_('settings_api_tokens_ct.form.new_token')}
        </button>
      {:else}
        <div class="create-form" transition:slide={{ duration: 160 }}>
          ...
          <div style="display:flex;gap:8px;margin-top:8px">
            ...
            <button class="btn btn-primary" style="flex:2" on:click={createNewToken} disabled={creating}>
              {creating ? $_('settings_api_tokens_ct.form.creating') : $_('settings_api_tokens_ct.form.create')}
            </button>
          </div>
        </div>
      {/if}
