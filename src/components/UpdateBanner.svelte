<script>
   ...
   *            for install action). Also posts a one-shot OS notification
   *            alongside the in-app banner so a user who backgrounds the
   *            app still sees the shade notification.
   *   PWA:     shows PWA-bundle-update banner when a new service worker
   *            is ready (tap "Reload" activates it) — see pwa-update.js
   *            for the trigger.
   *
  ...
  const NOTIFIED_KEY = 'ct_updates_notified_version';
  ...
  async function turnOnChecks() {
    dismissOffNotice();
    const { setAutoCheck, checkForUpdate, setServerUpdateCheck } = await import('../lib/updates.js');
    setAutoCheck(true);
    try { await setServerUpdateCheck(true); } catch { /* not an admin, or offline */ }
  ...
  onMount(async () => {
    ...
    try {
      ...
      // in-app banner (both channels active means a user who backgrounds
      // the app still sees the shade notification, and returns to a
      // banner they can dismiss in one tap).
