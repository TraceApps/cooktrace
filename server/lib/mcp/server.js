 ...
 * pair per HTTP request. Every tool here is a plain DB read/write with
 * no per-session state to persist, so session management would just be
 * maintenance burden. If a future tool needs a session (long-lived
 ...
 * and identifies the caller. Ported from NutriTrace's / LiftTrace's
 * server/lib/mcp/server.js for CookTrace's MCP server.
 */
...
export async function handleMcpRequest(req, res) {
  ...
  const server = new McpServer(
    ...
    {
      // Advertise only tools capability; no resources/prompts yet.
      capabilities: { tools: {} },
    }
  );
