 ...
 *
 * Outgoing webhooks. When a subscribed event happens (a recipe is
 * cooked, the shopping list is fully checked off, a pantry item runs
 * out), fire a signed HTTP POST to each of the user's enabled webhooks
 * subscribed to that event. Off by default (WEBHOOKS_ENABLED=1 to turn
 * on). Ported from LiftTrace's server/lib/webhooks.js.
 *
...
export const WEBHOOK_EVENT_DESCRIPTIONS = {
  'meal.cooked':             'A recipe is logged as cooked, from the cook diary or the "Mark as Cooked" shortcut.',
 ...
 * user waiting for a real cook or shopping-list completion. Bypasses
 * the enabled/subscribed-events filtering, a user testing a webhook
 * wants to know the URL and secret actually work, regardless of
 * whether the row is currently enabled or which events it's
 * subscribed to.
 */
export async function sendTestWebhook({ userId, id }) {
  ...
  await _deliverWithRetry(row, 'test', { message: 'This is a test event from CookTrace.' });
