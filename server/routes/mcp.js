/**
 * server/routes/mcp.js — Model Context Protocol endpoint.
 *
 * Mounted at /api/mcp when MCP_ENABLED=1 in the server env. Off by
 * default, so no existing user sees any change. Write tools (log_cook,
 * add_shopping_item, check_shopping_item, update_pantry_stock)
 * additionally require MCP_WRITE_ENABLED=1 on the server AND the
 * calling token holding the `mcp:write` scope. Destructive tools
 * (create_recipe, add_pantry_item, delete_diary_entry,
 * remove_shopping_item) require MCP_DESTROY_ENABLED=1 AND `mcp:destroy`
 * scope AND every call to include confirm=true. Any missing gate and
 * the corresponding tools don't appear in tools/list.
 *
 ...
 * limit budget. Ported from NutriTrace's / LiftTrace's server/routes/
 * mcp.js — security logic is fully generic, only the tool set differs.
 */
...
// Parse ALLOWED_ORIGINS (comma-separated). Server-to-server MCP clients
// (Claude Desktop's HTTP bridge, stdio wrappers) send no Origin header
// and always pass. A browser-based client MUST be listed in
// ALLOWED_ORIGINS explicitly — there is no same-Host fallback, because
// trusting the Host header is exactly the pattern DNS rebinding attacks
// exploit (an attacker can point evil.example at 127.0.0.1 and the
// browser will send matching Origin + Host headers). See MCP spec on
...
router.get('/', (req, res) => {
  res.status(405).json({
    ...
    error: {
      code: -32000,
      message: 'GET not supported: CookTrace runs MCP in stateless mode. Use POST.',
    },
  });
});
