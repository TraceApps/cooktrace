...
- **Kitchen members can edit shared recipes, if the owner allows it.** A new switch in Settings → Kitchens, off by default and shown only to the Kitchen's owner, lets everyone in the Kitchen edit the recipes shared through it, not just each recipe's owner. It's for a household that cooks from one library: whoever tweaks the recipe updates it for everyone. Members can edit a recipe's content; deleting it, sharing it further, and its visibility and category stay with its owner, since categories are per person. Leaving the Kitchen, or the owner turning the switch off, takes the edit access away. Recipes shared one to one, outside a Kitchen, stay read-only.
