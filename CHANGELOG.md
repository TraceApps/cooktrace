...
- **Kitchen roles, so a household can share one recipe library without sharing the keys to the server** ([#52](https://github.com/TraceApps/cooktrace/issues/52)). A recipe shared into a Kitchen used to be read-only for everyone but its owner, so fixing a quantity while cooking meant asking the owner or making everyone an admin, which also hands out deleting users and restoring backups. Each member now has a role in that Kitchen, set by its Head Chef in Settings, Kitchens: a **Sous Chef** edits the recipes and cookbooks shared into the Kitchen, a **Line Cook** cooks from them and is what everyone keeps unless told otherwise. The role can be chosen as you invite someone, changed later per person, or set for the whole Kitchen in one tap. Deleting a recipe or a cookbook, sharing either onward, and a recipe's visibility, category, star rating and favourite mark all stay with its owner, because those are the owner's own filing and opinion. Recipes shared one to one, outside a Kitchen, stay read-only. Demoting someone, or removing them from the Kitchen, takes the access away immediately, and a recipe saved by anyone other than its owner says who touched it last. A Kitchen can also be handed to another member now (**Make Head Chef**), which is the way out of the old "owner cannot leave" dead end; the outgoing owner stays on as a Sous Chef. On the phone, editing a recipe shared with you goes straight to your server and says so when there is no connection, since your phone never holds a copy of someone else's recipe. Thanks to @herver1971 for the idea and the first implementation in [#53](https://github.com/TraceApps/cooktrace/pull/53).

- **CookTrace on your wrist.** A Wear OS app for watches running Wear OS 3 and up. The shopping list is its home: your list by aisle, ticked off with a trolley in one hand, which keeps working in a shop with no signal and goes up when there is some. Press Cook on the phone and the recipe arrives on the watch: the ingredients as a checklist to tick off as they go in, the steps as another, each one openable in full, and a step that says "simmer for 20 minutes" offers that timer rather than making you dial it. Several timers run at once, because a kitchen is a pan and an oven, and each buzzes on your wrist whether or not the app is still on screen. "I cooked this" writes straight into your cook diary. Either device can tick something off and the other follows, so the phone propped up on the counter and the watch on your wrist never disagree. Nothing is typed on the watch and nothing is set up there: the phone hands over your server address and a token when you sign in, and takes them away when you sign out. Browsing, searching and editing recipes stay on the phone, where there is room for them.

  **A timer you can read at a glance.** One running gives you the whole screen: a ring that empties as the time goes, the time left, and buttons for another minute, another five, or stop. Several gives you the list, soonest first. The ring and every row are green while there is time, amber when it is getting on and red at the death, going by the clock rather than by the fraction left, because a three hour braise is not urgent with five minutes to go. Tapping a timer opens it and never stops it, so a mis-tap on a wrist cannot throw away a braise, and a timer that rings says which dish it was instead of buzzing about nothing.

  **What is counting shows on the watch face** and in the launcher while it runs, without the app open, with the system drawing the countdown so nothing wakes up to tick it. With several running it follows the one finishing soonest and says how many others there are; tapping it opens the timers, where they are all listed.

  **Timers are shared with the phone during a cook.** Start, extend or stop one on either device and the other follows, and either can ring you, which is the point of having both. Only during a cook: pressing Cook is what opens it, and off a cook nothing crosses in either direction, so a watch in a drawer is never woken for a timer you set on the phone.

  **What you have on the go is visible from anywhere in the phone app.** Leave a recipe without ending the cook and a slim frosted bar sits above the page naming the dish and how long it has been going; tapping it goes back to it. With more than one on the go it says so and offers the list, so every tap does something.

- **The web app keeps working without a connection** ([#211](https://github.com/TraceApps/nutritrace/issues/211) in NutriTrace, the same idea here). A supermarket with no signal is the case this is built for: your shopping list still opens, you can add items, tick them off, edit and remove them, and clear what you have ticked, and it is all there when you come back into range. The same goes for the pantry (adding, editing, removing, marking in or out of stock), the cook diary, your recipes and the notes on them, and settings. Recipes, cookbooks, the pantry and the diary are read from what this browser has already seen, so you can cook from a recipe with no signal, pictures included. What you change is kept in the browser, survives closing the tab, and goes up on its own when the connection returns, as the very requests the app made, so your server does exactly what it would have done anyway. Two tabs cannot send it twice. Nothing is sent while you are offline, and the queue is cleared only once your server has confirmed it. The menu button shows an amber cloud while anything is waiting. If your server refuses one of your changes, that one is set aside and named in plain words ("Your server would not accept the item you added to your shopping list: ..."), everything else still goes up, and the reason is written to the diagnostics log; a server that is merely busy, or a session that has expired, is retried instead, so your work is never thrown away because a cookie timed out. Signing out sends what is waiting first, and asks before discarding anything it could not send. The installed app also survives a reload or a relaunch with no connection: it now keeps its own code in the browser, where before it only kept a fallback page and came back blank. Photos work offline too, everywhere you can attach one: what you cooked (from a recipe or straight into the diary), a recipe's own photo, a cookbook cover, and a pantry item, on adding and on editing. The photo travels inside the entry and becomes an ordinary file on your server the moment the queue goes up. A photo too large to hold says so rather than being lost. Your profile works offline too, picture included: choose one with no signal and it travels with the profile, becoming an ordinary file on your server when the queue goes up. Still needs a connection: importing recipes, sharing a recipe or a cookbook, kitchens and their members, Trace, and anything admin. Deliberately not built on Background Sync, which Safari does not have, so an iPhone behaves the same as everything else.
...
- **Ticking things off on the phone no longer wakes the watch once per tap.** Each change was sent across on its own, so ticking six ingredients in a row started the watch app six times. A burst now settles into one message; handing a recipe over still happens at once.
...
- **A change made with no connection is answered in the shape that screen expects**, so nothing looks broken while it waits. Logging a cook from a recipe, for instance, answers with the recipe, exactly as your server would.
...
- **A sync no longer empties the copy this browser keeps.** Everything was cleared once the queue went up, so losing signal again straight afterwards left you with nothing to look at until you were back online. Only what actually changed is dropped now.
...
- **What a row created offline became is now remembered on disk**, not just for as long as the page stays open. A sync that stopped halfway (the tab closed, the signal went again) could leave work queued against an id your server never had. Borrowed from NoteTrace, which has always done this.
