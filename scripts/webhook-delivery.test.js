...
test('signEnvelope produces a JSON envelope with event/timestamp/data and a matching HMAC-SHA256 signature', () => {
  const { envelope, signature } = signEnvelope('my-secret', 'meal.cooked', { recipe_id: 42 });
  const parsed = JSON.parse(envelope);
  assert.equal(parsed.event, 'meal.cooked');
  assert.deepEqual(parsed.data, { recipe_id: 42 });
...
test('signEnvelope is deterministic for the same input at the same instant but signature changes with the secret', () => {
  const a = signEnvelope('secret-a', 'pantry.out_of_stock', { pantry_item_id: 1 });
  const b = signEnvelope('secret-b', 'pantry.out_of_stock', { pantry_item_id: 1 });
...
test('sendWebhookRequest POSTs the exact envelope with the signature and event/delivery headers', async () => {
  ...
  const { url, close } = await withServer((req, res) => {
    ...
    req.on('end', () => {
      received = {
        method: req.method,
        headers: {
          signature: req.headers['x-cooktrace-signature'],
          event: req.headers['x-cooktrace-event'],
          delivery: req.headers['x-cooktrace-delivery'],
  ...
  try {
    const { envelope, signature } = signEnvelope('shh', 'shopping_list.completed', { items_count: 5 });
    await sendWebhookRequest(url, envelope, signature, 'delivery-123', 'shopping_list.completed');
    assert.equal(received.method, 'POST');
    assert.equal(received.headers.signature, `sha256=${signature}`);
    assert.equal(received.headers.event, 'shopping_list.completed');
...
test('sendWebhookRequest throws on a non-2xx response, so the retry loop in _deliverWithRetry actually retries', async () => {
  ...
  try {
    const { envelope, signature } = signEnvelope('shh', 'meal.cooked', {});
    await assert.rejects(() => sendWebhookRequest(url, envelope, signature, 'd-1', 'meal.cooked'), /500/);
  } finally {
    await close();
  }
});
...
test('sendWebhookRequest rejects when the receiver is unreachable (connection refused)', async () => {
  ...
  await close();
  const { envelope, signature } = signEnvelope('shh', 'pantry.out_of_stock', {});
  await assert.rejects(() => sendWebhookRequest(url, envelope, signature, 'd-2', 'pantry.out_of_stock'));
});
...
test('sendWebhookRequest does NOT follow a redirect (SSRF guard bypass regression check)', async () => {
  ...
  try {
    const { envelope, signature } = signEnvelope('shh', 'meal.cooked', {});
    await assert.rejects(() => sendWebhookRequest(url, envelope, signature, 'd-3', 'meal.cooked'), /302/);
