...
test('ordinary page content still starts a pull', () => {
  assert.equal(isPullSyncExempt(el(['recipe-card'], {}, el(['page-transition']))), false);
...
test('each draggable in the app is marked', () => {
  ...
  const recipe = read('../src/routes/RecipeEditor.svelte');
  assert.match(recipe, /class="ing-handle"\n\s*draggable="true"\n\s*data-no-pull-sync/);
  assert.match(recipe, /class="step-handle"\n\s*draggable="true"\n\s*data-no-pull-sync/);
  const cookbook = read('../src/routes/CookbookView.svelte');
  assert.match(cookbook, /setAttribute\('data-no-pull-sync', ''\)/, 'cookbook cards while reorderable');
  assert.match(cookbook, /removeAttribute\('data-no-pull-sync'\)/, 'and not when they are not');
  assert.match(read('../src/routes/Shopping.svelte'), /class="drag-handle"/);
  assert.ok(PULL_SYNC_EXEMPT.includes('.drag-handle'));
});
