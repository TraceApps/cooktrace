/**
 * Android's back button only knew how to go back a page, so it left pages
 * with a sheet or dialog still open. Open sheets, dialogs and overlays now
...
test('every modal overlay in the app is registered', () => {
  ...
  const innerPanels = [
    ...
    ['components/recipe/CookLogDialog.svelte', 'cl-box'], ['routes/Shopping.svelte', 'modal-clear'],
  ];
...
test('every hand-built backdrop and overlay is registered too', () => {
  ...
  const notLayers = new Map([
    ...
    ['routes/CookDiary.svelte', 'photo-overlay'],                    // the caption on a photo tile
    ['components/ui/BarcodeScanner.svelte', 'scanner-backdrop'],     // the browser scanner; Android uses Google's scanner screen
  ]);
  const files = walk(new URL('../src/', import.meta.url).pathname).filter((f) => f.endsWith('.svelte'));
  const missing = [];
  let checked = 0;
...
test('the shared components and hand-built sheets are registered', () => {
  ...
  assert.equal((read('../src/routes/Shopping.svelte').match(/use:closeOnBack=/g) || []).length, 5);
  assert.equal((read('../src/routes/Recipes.svelte').match(/use:closeOnBack=/g) || []).length, 5);
